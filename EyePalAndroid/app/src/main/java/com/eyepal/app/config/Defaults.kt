...
object Defaults {
    ...
    const val QUICK_MODEL_PROVIDER = "gemma"
    const val QUICK_GEMMA_MODEL_KIND = "e2b"
    const val TRANSLATION_TARGET = "zh"
    const val FACE_MATCH_THRESHOLD = 0.60f
    const val FACE_MATCH_MARGIN = 0.05f
    ...
    const val SAME_IDENTITY_SIMILARITY = 0.85f
    const val SAMPLE_DISTINCT_SIMILARITY = 0.995f
    const val ENROLLMENT_MINIMUM_FACE_SIZE = 80
    const val BORDERLINE_KNOWN_THRESHOLD = 0.85f
    const val UNKNOWN_SUGGESTION_FRAME_THRESHOLD = 6
    const val UNKNOWN_SUGGESTION_MIN_INTERVAL_MS = 10_000L
    const val MINIMUM_ENROLLMENT_SAMPLES = 1
